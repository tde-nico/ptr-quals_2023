            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1839constructorimpl = SegmentOrClosed.m1839constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1839constructorimpl = SegmentOrClosed.m1839constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1844isClosedimpl(m1839constructorimpl)) {
                break;
            }
            Segment m1842getSegmentimpl = SegmentOrClosed.m1842getSegmentimpl(m1839constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= m1842getSegmentimpl.getId()) {
                    break;
                } else if (!m1842getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    continue;
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1842getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1842getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m1842getSegmentimpl.remove();
                }
            }
            z = true;
            continue;
        } while (!z);
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1842getSegmentimpl(m1839constructorimpl);
